...
from tensor_to_pycolmap import batch_matrix_to_pycolmap, pycolmap_to_batch_matrix, get_image_pose_world_to_cam
...
# Helper functions for Extrinsics Storage
...
def store_extrinsics_information(recon, out_path, extrinsics,
    ...
    for image_id, image in image_items_sorted:
        ...
        C_w = -R_wc.T @ t_wc
        T = np.hstack((R_wc, np.vstack(t_wc)))
        w2c_mat = np.vstack((T, np.array([[0.0, 0.0, 0.0, 1.0]])))
    ...
    Ext_w2c = np.stack([r["extrinsics_w2c"] for r in image_records], axis=0)
    ...
    ext_w2c_np = extrinsics.cpu().detach().numpy()
    print("SHAPE: ", ext_w2c_np.shape)
...
def run_vggt_with_ba(
    ...
    store_extrinsics_information(reconstruction, out_path, pred_extrinsic,
                                 device=device, 
                                 camera_type=camera_type)
    ...
    return pred_extrinsic, mean_error