...
def calculate_reproj_error(points3d,
    extrinsics,
    intrinsics,
    tracks,
    max_reproj_error: int | None = None):
    ...
    if max_reproj_error is not None:
        ...
        print(reproj_mask.shape)
    print(projected_diff.shape)
    print(np.median(projected_diff))
    return projected_diff.mean()

def batch_np_matrix_to_pycolmap_new(
    points3d,
    extrinsics,
    intrinsics,
    tracks,
...
def batch_np_matrix_to_pycolmap(
    ...
    for vidx in valid_idx:
        # Use RGB colors if provided, otherwise use zeros
        rgb = points_rgb[vidx] if points_rgb is not None else np.zeros(3)
        pid = reconstruction.add_point3D(points3d[vidx], pycolmap.Track(), rgb) # include output pid
    ...
    for fidx in range(N):
        ...
        # NOTE point3D_id start by 1
        # for point3D_id in range(1, num_points3D + 1):
        for local_idx, point3D_id in enumerate(point3D_ids):
            original_track_idx = valid_idx[local_idx] #valid_idx[point3D_id - 1]
            ...
            if (reconstruction.points3D[point3D_id].xyz < max_points3D_val).all():
                if masks[fidx][original_track_idx]:
                    ...
                    # add element
                    track = reconstruction.points3D[point3D_id].track
                    track.add_element(image.image_id, point2D_idx)
                    point2D_idx += 1
