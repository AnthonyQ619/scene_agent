...
STEP 5: Do Not Detect Feature Tracks
- Reasoning: This specific scene has very large view point changes and low/inconsistent lighting, so any estimated features will lead to any 
  matching correspondences or tracks to be very inaccuarte. So we opt for a detector free pipeline in this scenario since features will be inherently inaccurate.

STEP 6: Reconstruct the Scene now that we have the estimated Camera Poses. We will use the Sparse3DReconstructionVGGTNoFeatures Module in this scenario
