...
def read_camera_flow(image_paths, calib_path):
    ...
    # if intrinsics is None:
    #     return {
    #     "mean_flow": float(np.mean(mean_flow)),
    #     "median_flow": float(np.median(mag)),
    #     "p75_flow": float(np.percentile(mag, 75)),
    #     "p90_flow": float(np.percentile(mag, 90)),
    # }
...
image_dir = "/home/anthonyq/datasets/DTU/DTU/scan14" #Path("/home/anthonyq/datasets/DTU/DTU/scan14")
calib_path = "/home/anthonyq/datasets/DTU/DTU/calibration_DTU_new.npz"
read_camera_flow(image_dir, calib_path)

# image_dir = Path("/home/anthonyq/datasets/ETH/ETH/bridge/images/dslr_images_undistorted")
# out_dir = Path("flow_outputs")
# out_dir.mkdir(exist_ok=True)

# image_paths = sorted(image_dir.glob("*.png"))

# for i in range(len(image_paths) - 1):
#     flow = estimate_flow(str(image_paths[0]), str(image_paths[1])) 

# print("Finished dataset")
# result = estimate_motion_from_flow(flow, K)
